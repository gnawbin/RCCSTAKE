async function main() {
    ...
    const contract  = await hrt.ethers.getContractFactory("contracts/RCCToken.sol:RCCToken");
    const contractRCCToken = await contract.deploy();
    await contractRCCToken.waitForDeployment();
    console.log("成功部署合约:", contractRCCToken.target);
  }
